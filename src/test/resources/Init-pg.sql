...
CREATE
index index_code  ON Member (dimid,code,status,membertype);
CREATE
index index_position  ON Member (position asc);
...
CREATE
index index_attr  ON ATTR (dimid,attrName);
-- ATTRValue表
...
CREATE
index index_attrvalue  ON attrvalue (attrid,memberCode);
-- Cube表
